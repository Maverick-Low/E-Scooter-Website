    ...
    if request.method == "GET":
        ...
        if form.validate_on_submit():
            ...
            hashed_password = bcrypt.generate_password_hash(
                form.password_1.data).decode('utf-8')
            user_obj = models.User(
                username=form.username.data, email=form.email.data, password=hashed_password, admin=False)
...
def weekly_income():
    ...
    # Stores the sum of the price for each booking within the corresponding week
    sums = [0] * 6
...
def remove_available(location):
    ...
    scooter_to_remove = models.Scooter.query.filter_by(
        LocationID=param[0], in_use=False).first()
...
def payment():
    ...
    if request.method == "GET":
        ...
        currentUser = models.User.query.filter_by(
            email=session.get('email')).first()
        exists = db.session.query(models.Card.UserID).filter_by(
            UserID=currentUser.id).first() is not None
        if (exists == True):
            currentUserCard = models.Card.query.filter_by(
                UserID=currentUser.id).first()
            autoFilledName = currentUserCard.name  # Retrieves name of user
            autoFilledCardNumber = f.decrypt(currentUserCard.cardnum).decode(
                "utf-8")  # Retrieves user's card number
            autoFilledExpiry = currentUserCard.expiry.strftime(
                "%m/%Y")  # Retrieves user's card expiry date
            ...
            form = Payment(formdata=MultiDict([('name', autoFilledName), ('card_number', autoFilledCardNumber), ('expiry_date', autoFilledExpiry),
                                               ('address_line_1', autoFilledAddressLine1), (
                                                   'address_line_2', autoFilledAddressLine2), ('city', autoFilledCity),
        ...
        if form.validate_on_submit():
            ...
            if(len(value) != 0):
                arr2 = [form.name.data, form.card_number.data, form.expiry_date.data,
                        form.address_line_1.data, form.address_line_2.data, form.city.data, form.postcode.data]
                x = form.card_number.data.encode()
                encryptedCard = f.encrypt(x)
                user = models.User.query.filter_by(
                    email=session.get('email')).first()
                card_obj = models.Card(UserID=user.id, name=arr2[0], cardnum=encryptedCard,
                                       expiry=arr2[2], address1=arr2[3], address2=arr2[4], city=arr2[5], postcode=arr2[6])
...
def guest_payment():
    ...
    arr = request.args['hours']
    form = Guest_Payment()
    locations = ['Trinity Centre', 'Train Station',
                 'Merrion Centre', 'LRI Hospital', 'UoL Edge Sports Centre']
    if request.method == "GET":
        # In order to display the location that user is reserving scooter from on payment screen
...
def staff_add():
    ...
    if request.method == 'POST':
        new_scooter = models.Scooter(
            LocationID=int(form.location.data), in_use=False)
...
def staff_edit_scooter(scooterID):
    ...
    if request.method == 'GET':
        form = EditScooter(formdata=MultiDict(
            [('location', scooter.LocationID), ('in_use', scooter.in_use)]))
